...
{$_SESSION["test"]='consulterReser';
    include "../Core/reservationAdmin.php";
include "../config.php";
    $comptes = new reservationAdmin();
$liste = $comptes->getReservation('cinUtilisateur',$_SESSION["id"]);
  ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <div class="main-content">
                           ...
                           <div class="table-responsive table--no-card m-b-30">
                                    <table class="table table-borderless table-striped table-earning">
                                        ...
                                        <tbody>
                                        <?PHP
                                            foreach ($liste as $row) {if($row['acceptation']!=2)
                                                {
		                                ?>
		                                <tr>
			                                ...
			                                <td><?PHP echo $row['tempsDebut']; ?></td>
                                            <td><?PHP echo $row['tempsFin']; ?></td>
                                            <td><?PHP if ($row['acceptation']==0) echo 'en attente';
                                            else if($row['acceptation']==1) echo 'approuve';  
                                             ?></td>
                                            <form action="modifierReser.php" method="POST" >
			                                <td>
                                            <input type="hidden" name="ref" value="<?PHP echo $row['ref'];  ?>">
                                             <input type="submit" name="modifier" value="modifier" <?PHP if($row['acceptation']==1) echo 'checked'; ?>> 
                                             </form>
                                            </td>
